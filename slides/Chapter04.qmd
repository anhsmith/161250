...
| *Fail to reject $H_0$* | Correct result   | Type II error     |



## Truth and outcomes

- The probability of making a Type I error is set by the experimenter *a priori* (beforehand) as the significance level, $\alpha$.

- For example, if we set $\alpha$ = 0.05, we know that we have a 5% chance of rejecting $H_0$ even when it is true (the Type I error rate is 0.05).

- The probability of retaining $H_0$ when it is false is usually represented by $\beta$ (“beta”).

- We usually do not know what this is. It depends on lots of things, like $\sigma$, $n$ and also what the actual alternative hypothesis is.


## Concept of power

The ***power*** of a hypothesis test is the probability of rejecting the
null hypothesis if it is indeed false.
...
-   If we can assume that two populations have the same spread, we
    perform a **pooled-sample t-test** in which the pooled variance (common to both populations) is estimated as
...
-   I can be useful to transform data to make the distribution more like a 'normal'
...
-   Perform EDA in addition to computing coefficients of skewness or
    D-Statistics etc
-   Small datasets may look skewed, but might not be.
-   Transformations make the data more difficult to interpret.
-   In the context statistical models, we are only interested in whether the residuals (deviations from the fitted means) are normal—not whether the original variable is normal.
...
- estimating sampling variation using the standard error,
...
Non-parametric tests can be used for non-normal data.
