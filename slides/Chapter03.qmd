...
gives the cumulative probabilities up to and including the quantile $k$.
...
The probability density function (pdf) gives the probabilities as an
...
$$F_{X}(x)=\int _{-\infty }^{x}f_{X}(t)\,dt$$ The relationship between
...
For small samples, we do not expect many outliers but the shape might be
difficult to judge.

```{r}
set.seed(1234)
dfm <- data.frame(x=rnorm(50, mean=80, sd=12))
p1 <- ggplot(dfm, aes(x))+geom_histogram(aes(y=..density..), colour=1)
p1 <- p1 + stat_function(fun = dnorm, args=list(mean=mean(dfm$x), sd=sd(dfm$x)), geom = "line")
p2 <- ggplot(dfm, aes(x))+geom_boxplot()
library(patchwork)
p1/p2+ plot_layout(heights = unit(c(6, 1), c('cm', 'null'))) 
```
...
Symmetrical distributions such as normal (read corresponding to zero
skew) are poor fits.

Lognormal also does not fare well
...
-   Look for the cases in the literature; see whether any mechanistic
    justification is available in particular.

-   Large amount of data needed, often over 300.

-   If observed data is a mixture from 2 or more distributions, no point
    in fitting a single distribution.

    -   Do adequate EDA of the data

-   Be liberal for small sample sizes but fit models based on normal and
    employ techniques such as the transformation method (covered later)
