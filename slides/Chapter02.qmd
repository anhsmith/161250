...
-   I haven't seen these used anywhere since highschool!

```{r,  warning=FALSE, message=FALSE, comment=NA }
library(aplpack)
set.seed(1234)
stem.leaf(rnorm(50, mean=100))
```
...
p1 <- ggplot(dfm, aes(X))+ 
  ...
  ggtitle("Heights sum to n")
p2 <- ggplot(dfm, aes(X))+ 
  geom_histogram(bins = 20, aes(y = (..count..)/sum(..count..))) + 
  ylab("relative frequency") +
  ggtitle("Heights sum to 1")
p3 <- ggplot(dfm, aes(X))+ 
  geom_histogram(bins = 20, aes(y=..density..)) +
  ggtitle("Heights x widths sum to 1")
library(patchwork)
p1+p2+p3
```
...
-   Lots of examples on the web (particularly with `ggplot2`)
...
-   Pearson correlation coefficient is a measure of **linear** relationship
    ...
    -   Case b: Negative linear relationship ![](./img/2_41.png)
