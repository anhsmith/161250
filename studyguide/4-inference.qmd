...
Given this random sampling variation, here are some explanations for some commonly used measure of uncertainty:
...
Let's say we have some data and we wish to test the idea that the data came from a population that is normally distributed. The null hypothesis is that the population conforms a normal distribution. We calculate a test statistic that quantifies the degree of *departure* of the data from what we'd expect if the null hypothesis were true, and the population was indeed normal. 
...
https://shiny.massey.ac.nz/kgovinda/demos/demo.critical.values/ for exploring their probability densities and quantiles.
...
### Checking the adequacy of a transformation
...
| D-statistic | vehicle | vehicle$^2$ | $\sqrt{vehicle}$ | loge(vehicle) |
|:------------|:--------|:------------|:-----------------|:--------------|
| D$_{1}$     | 0.2724  | 0.2831      | 0.1308           | -0.0945       |
| D$_{2}$     | 0.2973  | 1.0615      | 0.0978           | -0.0611       |
| D$_{3}$     | 0.0714  | 0.2273      | -0.0183          | -0.1092       |

```{r}
#| echo: true
#| label: "fig-rangilval"
#| fig-cap: "Letter values of Vechicle data"
...
rangitikei |> 
  ...
  ggplot() +
  aes(y=people, x = time, color=time) +
  geom_boxplot() +
  facet_wrap(~temp) +
...
rangitikei |> 
  group_by(time,temp) |> 
  summarize(medians = median(people), 
...
rangitikei |> 
  ...
  group_by(time,temp) |> 
  summarize(medians = median(trans.ppl),
...
<!-- Also using `exactRankTests` package. Try- -->
