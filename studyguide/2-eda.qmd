...
> *"How do all these unusuals strike you, Watson?"*<br>
...
> _The Adventure of the Abbey Grange_

## What is EDA?

Exploratory Data Analysis (EDA) is just not producing various graphical displays. Exploratory analysis is detective in character, and even called as quantitative and graphical detective work by John @tukey1977 in his book. As in a detective story, the circumstances surrounding a crime can be accidental. In other words, confirmatory analysis, which is judicial in character and relies on statistical models, must follow after EDA. EDA is the foundation which uncovers indications or clues for confirmatory analysis.

The best way to learn EDA is trying out many things and finding which works and which doesn't. Experts remark that we often learn less from an expertly done analysis than from one where something is missed out by not trying.

@Chatfield listed the following guidelines for tackling statistical problems.

1.  Do not attempt to analyse the data until you understand what is being measured and why. Find out whether there is prior information such as are there any likely effects. Find out how the data were collected.

2.  Look at the structure of the data.

3.  The data then need to be carefully examined in an exploratory way before attempting a more sophisticated analysis.

4.  Use your common sense at all times.

Evidently the above rules warn that data analysis can never be a cook book recipe, and the role and experience of the analyst is important in making sense of data and providing interpretations. If data analysis can be reduced to a unique set of rules, then it shouldn't be hard to write a computer program which will do all the analysis and interpretations for us. Statistical thinking in the context of the data is important for interpreting the results and making useful tentative conclusions.

@Boxonscience remarked that data analysis can be viewed as a sub-iterative step in the process of scientific investigation illustrated below: $$\boxed{{\text {Tentative Model}}} \begin{array}{c} {Inference\to } \\ {\leftarrow Criticism} \end{array}\boxed{{\text {Tentative Analysis}}}$$ In the inferential stage, the analyst acts as a sponsor for the model. Conditional on the assumption of the truth of the model, the best statistical procedure is selected. EDA is essential to justify the assumptions of the model. After completing the analysis, the analyst turns to a critic (from being the sponsor of the model). Contrary to the assumptions made, the model may be faulty in one or more ways. Hence appropriate diagnostic checks are applied involving various kinds of residual analysis. EDA is important for such model criticism. In subsequent chapters, you will find further EDA tools for model criticism.

After a data set has been collected, the next stage of a statistical analysis involves using descriptive techniques to understand the nature of the data in relation to the problems in hand. Descriptive techniques include: summary statistics, tables, and graphs. Employing these methods is commonly referred to as exploratory data analysis (EDA).

EDA can be thought of as being comprised of four stages: the data familiarisation stage, the error checking stage, the data understanding stage, data visualisation and analysis stage. The last stage of analysis requires experience and involves creativity to find a key to a problem or discovering something new which is not known before the analysis.

## Some basics before EDA

Before one can even think of analysing a data set, one must be familiar with the data. At the very least, one must know:
  
  1.  what population can we make inferences about based on this data,

2.  what sampling strategy was used, and

3.  how are the variables in the data set recorded, and are they directly measuring the quantities of interest?
  
  As discussed in Chapter 1, one must be alert to sampling biases that may result in the sampled data being drawn from a different population to the stated population of interest. Particular care is needed if one is using data from another source (e.g. a similar study commissioned at a different location).

The sampling strategy is also crucial to which inferences can be made. For instance, if the data were sampled with a gender stratified design assuming 50% of the population was female, no inference can be made about the percentage of each gender; this has been fixed by the sampling strategy.

We should also know which measuring devices were used to collect the data; in particular, their accuracy, and whether or not they are susceptible to any biases.

Finally, what exactly does each variable record? Are the variables contained in the data the actual variables we are interested in, or merely convenient surrogates?
  
### Error checking
  
  The data are assumed to be in some sort of electronic form; e.g. text file, spreadsheet, database, etc. Before any time is expended on analysing the data set, it is prudent to check if the data have been recorded correctly. If one has access to the raw data (e.g. returned questionnaires), one can check each observation by hand. If the raw data are not available, then the best one can do is to look for observations whose values are out of allowable ranges, or are very suspicious. A common problem occurs with the coding of missing information. Missing values are commonly recorded as a blank, `*`, `-1`, `-999`, or `NA`. Only one type of coding should be used. The use of `NA` is preferred since this is easier to spot. The use of blanks is particularly dangerous when one is exporting data from a spreadsheet to a text file. When the data from the text file is read into a computer package, a missing value may be skipped and the value from the next observation read in its place. When the data set is large, looking at a data set one observation at a time may not be feasible. In this case, plotting the data may show any observations with illegal values.

### Understanding data
  
  After any data errors have been found and corrected, the data understanding stage commences. This stage involves tabulating, summarising, and plotting the data in many ways to gain insight into the data set. We typically want to: examine the distributions, or shapes, of individual variables; compare different groups of data, with special emphasis on location and scale; and discover any trends and relationships exhibited between pairs of variables. During this procedure, plots are typically constructed for ones own understanding. Excessive time should not be spent on the stylistic appearance of the plots, e.g. adding elaborate titles, axis labels, legends, etc. This will be done in the data presentation stage.

There is a danger, at this stage of the analysis, to fall into the trap of **data snooping**. Data snooping is the practice of selecting hypotheses to test based on the results of the EDA, and then testing them on the same data. For instance, suppose that boxplots of the income of individuals in different age groups show that youngest and oldest age groups are the most different. If one elects to perform the hypothesis test for the difference between these two groups based on the EDA, the resulting $p$-value will not be valid. Why is this so? The $p$-value is the probability that the observed difference between the groups is as large as it is by chance; in this case the difference we have observed is the largest of several inter-group distances. As such, the probability that it exceeds a certain value by chance is larger than the $p$-value reported by a test that assumes there are only two groups. However, if we had decided to perform this particular test before performing the results of the EDA the resulting $p$-value will be valid.

In the forthcoming sections, a quick review of many of the common data visualisation tools useful for EDA is presented.

## Categorical Data

There are many methods for examining data that is categorical in nature. We will explore several methods, including: tables; bar charts; dot charts; and pie charts. These tools are covered in foundation courses and hence we will briefly review them.
...
ggplot(Snodgrass, aes(x=Ceramics)) + geom_bar()
```
...
It is easy to spot that the Ceramics findings were largely from the first segment. This EDA conclusion can be drawn from all of the above plots. However, these bar plots do not rate the same as a "presentation style graphic". For example, colour can be a distraction. If you are reading this notes in printed form, the colours used may not be clear to you.
...
4.  the graph should display data variation, not design variation, i.e. visual changes in the graph should be due to changes in the data, not changes in way the data are drawn;

5.  the number of dimensions used in the graph should not exceed the number of dimensions in the data;

6.  graphs must not quote data out of context, e.g. a monthly drop in the murder rate of a city should be plotted within the context of the murder rate over the last few years, or murder rates in comparable cities over the same period.

A final point concerns the use of colour. In many sorts of graphs, data types are differentiated by the use of colour. This practice is perfectly legitimate, but one should be aware that a surprising proportion of people have impaired colour perception, e.g. they cannot distinguish red and green. Also, a graph that is printed in colour may be photocopied as black and white. A remedy to these problems is to use different plotting symbols, line types, or shading, in conjunction with different colours. One should always make sure that the colours selected are clearly visible in the medium in which they are presented, e.g. colours on a computer monitor can appear quite different when projected onto a screen in a lecture theatre, or when printed onto paper.
...
4.  have a high data to ink ratio, i.e. convey the greatest amount of information in the shortest time with the least ink in the smallest space;
...
7.  reveal detailed information secondary to the main trend, e.g. the location of outliers, variability of the response.
...
<!-- For a number of other presentation style graphics, see http://vita.had.co.nz/papers/prodplots.pdf -->
...
Stem plots are useful for raw data collection when done manually. These plots preserve the raw rounded data and can show the distribution and gaps. A simple version stem-and-leaf display for the Length data is shown below.

```{r}
stem(Snodgrass$Length, scale = 2)
```

This plot essentially shows the rounded data depending on the `scale` selected. Compare the stem plot with the sorted raw data shown below. Also explore the stem plot by changing the `scale` value.

```{r}
sort(Snodgrass$Length)
```

An enhanced version of the stem and leaf plot can be obtained using the `R` package `aplpack`. Try:

```{r, results='hide'}
library(aplpack)
stem.leaf(Snodgrass$Length)
```

You will notice that this enhanced plot indicates the stem in which the median is located. This is shown in the first column enclosing the number of observations in this stem by parentheses. For a batch of 91 numbers, the median is the 46th observation namely 14.5. This number falls in the median stem indicated as (13). The other numbers on the left indicate the cumulative counts from the lowest stem or from the largest stem. Thus, the numbers 2, 3, 6, 10, 22, 33 indicate that many observations up to, and including, that particular stem.

In summary, stem-and-leaf displays contain much information, so much so that, at times, they seem 'busy' and messy. If they are carefully studied, they can reveal the overall shape of the distribution and peculiarities such as gaps, the number of modes and the position of the median. Peculiar values are also apparent.

### Histograms

A stem-and-leaf display contains so much information that it may appear cluttered. A simpler display can be formed by drawing blocks over the class intervals **so that the areas of the blocks are proportional to the frequencies in the intervals**. The resulting display is called a **histogram** and is a very common type of display. Histograms are designed for continuous data or when discrete data has been grouped. It can be used for discrete ungrouped data, but a bar chart would be more suitable as such a display does not mislead the reader into thinking that values exist other than at the centre of the intervals. @fig-hist1 shows the histogram of Length. Clearly the distribution is far from symmetric and left skewed. The bimodal pattern also suggests subgrouping.
...
Histograms reveal similar patterns as stem-and-leaf displays but they are not as cluttered because they do not show the individual values.

It is sometimes helpful to divide the frequency by the sample size to obtain **relative frequencies** . These have the advantage that they sum to unity so that they are closely linked to the probabilities which have the same two basic properties namely (i) Each probability is non-negative, and (ii) The sum of the probabilities = 1. It is easy to plot a relative frequency histogram in place of a frequency one, since this does not alter the shape but merely changes the scale (which is now relative frequency per horizontal unit). @fig-hist2 shows the relative frequency (density) histogram of Length.

```{r}
#| echo: true
#| label: fig-hist2
#| fig-cap: 'A typical relative frequency histogram'
...
Graphs such as the one shown in @fig-hist3a are often preferred in biological sciences and other disciplines where a 'smooth curve' is preferred to the boxy histogram representation or to a frequency polygon.
...
ggplot(Snodgrass) + aes(Length) + geom_density()
```
...
symmetryplot <- function(x)
{
  ...
  m <- length(x) %/% 2
  sx <- sort(x)
  dfm <- tibble(X= median(x) - sx[1:m], Y=rev(sx)[1:m] - median(x))
  p1 <- ggplot2::ggplot(dfm, aes(x=X, y=Y)) + geom_point()
  p1 <- p1 + geom_line(aes(x=X, y=X)) + theme(aspect.ratio=1)
  p1+xlab("Distance below median")+ylab("Distance above median")
}
```
...
The points do not plot about the $Y=X$ or $45^\circ$ line but diverge from it, which indicates lack of symmetry.
...
A **boxplot** (or sometimes a *box-and-whisker plot*) is a visual display of the 5-number summary using which we can assess the skew in the data. The components of a boxplot is explained in @fig-boxplt.
...
The box shows the positions of the two fourths or hinges and we mark the median with a plus (+). As 50% of the observations fall between the fourths or hinges, the position of the box is important and is the first thing that catches our eyes. If the median is in the centre of the box, this suggests that the distribution may be symmetric; if the median is near one end, the distribution is skewed. In particular, if the median is in the *right* side of the box, it implies that the middle 50% of the data are skewed to the *left*. For right skewed data, the median will be placed at the left end of the box (as against in the centre of the box).

After having considered the 'centre' of a distribution, which we have measured by the median, and looked at where the bulk of the data lies, measured by the fourths; then we are particularly interested in the strange, unusual values. They could be obvious mistakes (incorrectly coded, or decimal places in the wrong place, transposition of digits such as 91 instead of 19) or incorrect measurements or recording. These last two possibilities often occur before the data gets to the statistician so that it may be difficult to check whether these mistakes have occurred. Apart from obvious errors, we can only point to 'far out' values as being strange and we term them **'outliers'**. To help decide this matter, we define a few more terms, of which the most common one is the **step**= 1.5$\times$F-spread. The Step is indicated in @fig-boxplt but not to scale.

Any observation which is more than one step outside of the upper (or lower) hinge can be considered as a **possible** **outlier** and is represented by a star (\*) in some software). If the observation is more than two steps outside of the hinges it can be considered as a **probable outlier** (and is represented by (`0`) in some software). Note that many software fail to distinguish probable outliers from possible ones.

Dotted lines are drawn from the box to the furthermost points which are still less than a step away and these points, one on each side of the box, are called **adjacent values** . These points are illustrated in @fig-boxplt. The lines joining the box with the adjacent values are called **whiskers**. Again the position of the median relative to the adjacent values or the outliers gives an indication of skewness or symmetry in the data.

Boxplots are ideal for comparing two batches for they give a visual summary of certain main features of the distributions but they are not as cluttered as stem-and-leaf displays. Indeed, their simplicity enables them to be used to compare a number of batches. Common sense needs to prevail, for there is a limit to the amount of information that the eye can take in. Up to 10 or 12 boxplots can be plotted, preferably on the same scale, but more than that could be confusing.

Shown in @fig-boxplt1 are the boxplots of Length for the Inside-Outside groupings. Note that the boxplots have been constructed on the same scale, i.e. they have common (Length) axis.
...
ggplot(Snodgrass) + 
  aes(y = Length, x = Inside, fill = Inside) + 
  geom_boxplot() + 
  coord_flip()
```

The boxplots in @fig-boxplt1 show that the general shape of the distribution for Length is broadly similar. In each case the median is not at the centre of the box which confirms the opposing nature of skew for the two groups. The whole box for the Outside group is shifted to the right of that for the Inside groups indicating that Length is longer for the Outside group. The outliers pertain to the Inside group only. Overall, that the '5-number summary' values for the Outside group are larger than those of the Inside group.

For large datasets, just a summary based on five positions is not representative. So we chop up the data by repeatedly halving the distance from the extreme values and get the **letter values**, namely, $M$ (median), $F$ (fourth), $E$ (eighth) and so on. Letter values are defined by their **depth**, or distance from the nearer edge of the ordered data.

`median`: found at depth $d(M) = (n + 1)/2$ where $n$ is the number of observations

`fourths`: found at depth $d(F) = \{\verb"int"[d(M)] + 1\}/2$ where `int` refers to the integer part function.
...
Note the M, F and E could be termed measures of **location**. It is also helpful to have some measures of **spread** which indicate the variability in the distribution. One such measure, the **F-spread**, is the distance between the two F (fourths) values. Finally, the **range** is the distance between the two extreme points.

Another useful measure of location is obtained by averaging the two hinges which is termed the **mid-fourth** (or **mid-F**). If the distribution were perfectly symmetrical, the mid-F would be the same as the median. If mid-F is smaller (larger) than the median, the distribution is skewed to the left (right). Note that the mid values can be found at all depth.
...
## Cumulative Frequency Graph and Quantiles
...
# rangitikei1 <- rangitikei[-which.max(rangitikei$people),]
...
@fig-marginbp2 shows the number of people against the number of vehicles seen as well as the distribution of each variable along the axes using boxplots (these are often referred to as *marginal distributions*). The first plot in @fig-marginbp2 (A) shows the scatter of all points, whereas the second one (B) shows the scatter with the very large value (Observation \#26) removed. In the first plot the outlier shows up quite clearly in both boxplots. In the second plot the boxplot associated with the variable 'people' still shows one point as an outlier, though this doesn't appear to be as extreme as the one in the first plot, and in the scatterplot the points fall close to a line which has a positive slope as is the case of @fig-marginbp2 (A). However, the distribution of each variable is skewed to the right.
...
$$r_{xy}=\frac{{\text {sum}(x - \bar x)(y - \bar y)}}{{\text{squareroot}[\text {sum}(x - \bar x)^2  {\text{sum}}(y - \bar y)^2 ]}}$$ $$= \frac{\sum (x-\bar{x})(y-\bar{y}) }{\sqrt{\sum (x-\bar{x})^{2}  \sum (y-\bar{y})^{2}  } } =\frac{S_{xy} }{\sqrt{[S_{xx} S_{yy} ]} }$$ 
...
For the Snodgrass data, the contour plot of East (X) vs, South (Y) vs. Area (Z) is shown in @fig-cntrplt. This plot shows that bigger house pits are inside the X and Y axis show the geolocations.

![A contour plot for the Snodgrass data](images/ContourPlot.png){#fig-cntrplt}
...
A biplot aims to show both the observations and variables in the same plot formed by the first two principal components. @fig-biplot shows the biplot for the pinetree variables and data.
...
For interpreting the biplot, consider the following patterns:
  ...
  1.  If points that are close to each other, it means observations with similar values. So you can spot the odd ones.

2.  The correlation between variables is indicated by the cosine of the angle between vectors displayed for the variables. Highly correlated variables will point in the same direction. If variables are uncorrelated, they will be at right angles to each other. For the pine tree data, we notice high correlation. This has implications in model building. We may not need all of the three predictor variables of Top. More will follow in later Chapters.

3.  The cosine of the angle between a vector and an axis indicates the contribution of the corresponding variable to the axis dimension. Look particularly the X-axis that represents the first principal component. The variables contribute somewhat the same.
